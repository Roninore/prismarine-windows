...
module.exports = (Item, registry) => {
  return class Window extends EventEmitter {
    ...
    shiftClick (click) {
      ...
      else if (this.type === 'minecraft:crafting_table') {
        console.log(item)
        if (click.slot === 0)
        {
     
          
          const craftCount = this.findMinCraftCount()
          console.log('Craft count',craftCount)
          item.count = item.count * craftCount
          console.log('Craft item',item)

          for (let i = 0; i < 10;i++)
            this.updateSlot(i,null)

          this.fillAndDump(item, this.inventoryStart, this.inventoryEnd, true)
        }
    ...
    findMinCraftCount() {//to refactor
      let min = 65
      for(let i = 1; i <= 9; i++)
        if (this.slots[i] && this.slots[i].count < min)
          min = this.slots[i].count
      return min !== 65 ? min : 0
    }//to refactor
    ...
    fillAndDump (item, start, end, lastToFirst = false) {
      ...
      if (item.count !== 0)
      {
        const freeSlots = this.findFreeSlots(item,start,end,lastToFirst)
        freeSlots.map(el => {
          this.updateSlot(el.slot,el)
        })
        console.log('Свободные слоты для заполнения',item.count,freeSlots)
      }
      console.log('Осталось', item.count)//to refactor
    }

    findFreeSlots(item,start,end,lastToFirst = false) {//to refactor
      const slotsNeed = Math.ceil(item.count / item.stackSize)

      const startI = lastToFirst ? end-1 : start
      const endI = lastToFirst ? start : end-1
      const dif = lastToFirst ? -1 : 1
      const comp = lastToFirst ? (a,b)=>{return a >= b} : (a,b)=> {return a <= b}

      const freeSlots = []
      let count = item.count
      
      console.log('FfS', startI,endI,dif,count)

      for(let i = startI; comp(i,endI); i+=dif)
      {
        if (freeSlots.length >= slotsNeed || count === 0)
          break
        if (!this.slots[i]) {
          const newItem = {
            ...item,
            count: count >= item.stackSize ? item.stackSize : count,
            slot: i
          }
          count -= newItem.count
          freeSlots.push(newItem)    
        }    
      }
      item.count = count
      if (slotsNeed < freeSlots.length)
        console.log('Не хватает места')
      return freeSlots  
    }//to refactor
